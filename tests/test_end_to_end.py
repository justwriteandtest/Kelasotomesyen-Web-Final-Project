...
def testEndToEnd(browser : webdriver.Chrome | webdriver.Edge | webdriver.Firefox | webdriver.Safari, itemList, addToCartMethod, removeFromCartMethod,  firstName, lastName, postalCode):
    ...
    with allure.step("Check out: Fill the shipping details"):
        cart.checkout()
    ...
    with allure.step("Cancel the check out"):
        shipping.cancelCheckout()

    with allure.step("Check out: Fill the shipping details again"):
        navbar.goToCart()
        cart.checkout()
    ...
    with allure.step("Finish the checkout process"):
